...
impl GetAllDto {
    fn from_auctions_and_items(bids: Vec<BidDto>) -> GetAllDto {
        GetAllDto {
            bids: bids.iter().map(BidDto::from).collect(),
        }
    }
...
impl<R: IAuctionRepository> GetBidsUseCase<R> {
    ...
    pub async fn execute(&self, auction_id: String) -> Result<GetAllDto, AppError> {
        ...
        let auction_id: Id<Auction> =
            domain::id::Id::try_from(auction_id.clone()).map_err(|_| {
                error!("Failed to parse auction_id = {}", auction_id);
                GetAuctionFailed(anyhow!(
                    "Failed to get auction for auction_id={};",
                    auction_id.clone()
                )?);
            })?;

        match self.auction_repository.get_all_bids(auction_id).await {
            Ok(bids_result) => {
